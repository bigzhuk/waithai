<?php
namespace Visits\Decorator;

use Framework\MainDecorator;

class Visits extends MainDecorator{
    const TEXT_CUT_LEGTH = 10;
    const MAX_TEXT_CUT_ATTEMPTS = 20;

    protected $homepage = 'visits.php';
    ...
    function setHomePage($home_page) {
        return $this->home_page = $home_page;
    }

    public function renderTable(array $new_rows) {
        $out = '';
        $out.= '<table>';
            $out.= '<thead>';
                $out.= '<tr>';
        ...
        foreach ($new_rows as $new_row) {
            $out.= '<tr>';
                ...
                $out.= '<td>
                            <a href = "'.$this->getHomePage().'?action=edit&id='.$new_row['id'].'">Редактировть</a>|
                            <a href = "'.$this->getHomePage().'?action=delete&id='.$new_row['id'].'">Удалить</a></td>';
            $out.= '</tr>';
        }
        $out.= '</table>';
    ...
    public function renderEditForm($row_new = null) {
        $id = isset($row_new['id']) ? $row_new['id'] : '';
        $form_action = 'actionIndex';
        ...
        $out = '';
        $out .='<form name="addForm" class="" action="'.$this->getHomePage().'.php?action='.$form_action.'" method="post">';
            $out .='<table><tbody>';
    ...
    public function renderAddButton() {
        $out = ' <a href = "'.$this->getHomePage().'?action=add">Добавить</a>';
        return $out;
    }

    public function renderEditButton() {
        $out = '<a href="http://'.$_SERVER['HTTP_HOST'].'/admin/'.$this->getHomePage().'">Редактировать новости</a>';
        return $out;
    }
